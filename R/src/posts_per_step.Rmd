...
df_posts_per_step %>% 
  ...
  ggplot() +
  aes(x = area_under_gini, y = area_under_ndcg,color = new_posts_per_step, group= seed, size = posts_with_no_views) + # , color = vote_count, shape = user_rating_function, fill = init_score) + 
...
df_posts_per_step %>% 
 # filter(steps <= 100) %>% 
  ...
  ggplot_cus() +
  aes(x = new_posts_per_step, fill =scoring_function, y = ρ) + 
  theme(text=element_text(family="Trebuchet MS")) +
  geom_boxplot()
