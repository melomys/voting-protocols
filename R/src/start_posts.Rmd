...
df_start_posts %>% 
  ...
  ggplot_cus() +
  aes(x = area_under_gini, y = area_under_ndcg, color = start_posts, group= seed, size = posts_with_no_views) + # , color = vote_count, shape = user_rating_function, fill = init_score) + 
...
df_start_posts %>% 
 # filter(steps <= 100) %>% 
  ...
  ggplot_cus() +
  aes(x = start_posts, fill =scoring_function, y = ρ) + 
