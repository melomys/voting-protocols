...
init_hack = df_init %>% 
  ...
  filter(gravity == 0) %>%
  filter(relevance_gravity == "Q&A") %>% 
  filter(scoring_function %in% c("Verallg. Hacker News")) %>% 
...
```{r}
help_tib = df_init %>% 
  mutate(scoring_function = "Verallg. Hacker News")
...
if(s)
{
  ggsave("/home/ludwig/Bachelorarbeit/voting-protocols/plots/dev_ρ.png")
}
...
df_gravity %>% 
  ...
  geom_boxplot(aes(fill = scoring_function, y = ρ,x= gravity)) + 
...
df_start_posts %>% 
  ...
  ggplot_cus() +
  aes(x = area_under_gini, y = area_under_ndcg, color = start_posts, group= interaction(seed,gravity, init_score, vote_evaluation, scoring_function), size = posts_with_no_views) +
