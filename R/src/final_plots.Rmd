...
grouped_full_model %>%
  ...
  #geom_point(aes(size = 1.5*rho), color = "grey20") + 
...
corr_full_model = tibble(Konsens_QandA = df_kons_0$rho, Konsens_News = df_kons_2$rho, Dissens_QandA = df_diss_0$rho, Dissens_News = df_diss_2$rho)
...
```{r}
ann_line<-data.frame(xend = 0.26, yend = 0.8, x = 0.3, y = 0.85, lab = "nuidtentraiunetriunednaiue" ,
    relevance_gravity=factor("News",levels=c("Q&A", "News")))

ann_text<-data.frame(x = 0.31, y = 0.85,
                     relevance_gravity=factor("News",levels=c("Q&A", "News")))
...
grouped_full_model %>% 
 ...
 # filter(init_score == 70) %>% 
  ...
  geom_curve(data=ann_line, aes(xend=xend, yend=yend, x=x, y=y),
    linejoin = "round", curvature = .45, arrow = arrow(length = unit(2, "mm"))
  ) +
  geom_text(data=ann_text, aes(x=x, y=y), hjust = "left",label = "Text") + 
...
df_full_model %>% 
  filter(user_rating_function == "Konsens") %>% 
  ggplot_cus() + 
  aes(fill = scoring_function, x = init_score, y = rho) +
...
grouped_full_model %>% 
  ...
  filter(init_score != 0) %>% 
  ggplot_cus() + 
  aes(y = rho, fill = vote_evaluation,x = scoring_function)  + 
...
df_deviation %>% 
 # filter(steps <= 100) %>% 
  ...
  #mutate(steps = factor(steps)) %>% 
  ggplot_cus() +
  aes(x = deviation_function,fill = scoring_function,y = rho ) + 
  theme(legend.position = "none") + 
  labs(x = "Zufallsabweichung", y = "rho") + 
  geom_boxplot() +
  #facet_wrap(~quality_dimensions, ncol = nrow(unique(df_dims[,"quality_dimensions"])))
  facet_grid(~relevance_gravity)
...
if(s)
{
  ggsave("/home/ludwig/Bachelorarbeit/voting-protocols/plots/dev_rho.png")
}
...
df_cus_steps$scoring_function = revalue(x = df_cus_steps$scoring_function, c("Verallg. Hacker News" = "Verallg. HN"))
...
cor(step100$rho, step500$rho, method = "spearman")
```
...
df_gravity %>% 
  #mutate(gravity = factor(gravity)) %>% 
  ggplot_cus() + 
  aes(fill = scoring_function, y = rho,x= gravity) + 
...
df_dims %>% 
  ...
  ggplot_cus() + 
  theme(legend.position = "right") + 
  aes(x = quality_dimensions, fill = scoring_function, y = rho) + 
...
df_userprob_cus$scoring_function = revalue(x=df_userprob_cus$scoring_function, c("Verallg. Hacker News" = "Verallg. HN"))
...
df_start_posts %>% 
  ...
  mutate(start_posts = factor(start_posts)) %>% 
  ggplot_cus() +
  aes(x = start_posts, y = rho,fill = scoring_function) + # , color = vote_count, shape = user_rating_function, fill = init_score) + 
...
if(s)
{
 ggsave("/home/ludwig/Bachelorarbeit/voting-protocols/plots/rating_dist1.png", device = "png")
}
```
```{r}
